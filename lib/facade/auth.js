...
Auth.prototype.validateAccessToken = function validateAccessToken(inputObject) {
    ...
    rules.addCustomSync(AuthConstants.messages.INVALID_ACCESS_TOKEN, function(){
        try{
 ...
 * it will return accessToken with newly created / or existing accountId
 *
 * @param inputObject
 * @param tokenAttributes
 * @returns {*}
...
Auth.prototype.register = function register(inputObject, tokenAttributes) {
    ...
    rules.addCustomAsync(AuthConstants.messages.ACCOUNT_ID_ALREADY_EXIST, function(){
        return new AccountLookup({accountId: inputObject.accountId}).fetch().then(function(accountObject){
