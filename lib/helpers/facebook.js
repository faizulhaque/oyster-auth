...
function facebook(inputObject) {
    ...
    this.requestOptionForApp = {
        uri: this.apiUrl + "/app?access_token=" + inputObject.username
    };
    this.accessToken = inputObject.username;
    this.requestOptionForMe = {
        uri: this.apiUrl + "/me?fields=" + this.requiredFailds.join(",") + "&access_token=" + inputObject.username
    }
}
...
facebook.prototype.validate = function validate() {
    ...
    return Promise.all([
        ...
        if(appData && parseInt(appData.id) !== global.socials[self.currentType].key) {
            throw new ErrorMessages.custom400Error(AuthConstants.messages.USER_NOT_BELONG_TO_FB_AP);
        }
        meData.id = parseInt(meData.id);
