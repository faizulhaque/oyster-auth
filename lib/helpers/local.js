...
function facebook(inputObject) {
    ...
    if(!inputObject.accountId) {
        throw new ErrorMessages.custom400Error(Util.format(AuthConstants.messages.REQUIRED, "accountId"));
    }
...
facebook.prototype.validate = function validate() {
    ...
    return new AccountLookupModel(self.loginObject).fetch().then(function(accountLookupObject){
        if(!accountLookupObject) {
            throw new ErrorMessages.custom400Error(AuthConstants.messages.INVALID_ACCOUNT_ID_PASSWORD);
        }
